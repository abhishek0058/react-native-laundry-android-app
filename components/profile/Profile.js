...
export default class Profile extends Component {
  ...
  cityChange = city => {
    console.log(city);
    const filteredHostels = this.state.hostels.filter(
    ...
    if (key == "Choose Hostel") {
      if (this.state.city == "null")
        return (
          <DropDown
            key={key}
            label={`Choose City First`}
            data={[]}
            updateHostel={hostel => this.updateHostel(hostel)}
          />
        );
      else
        return (
          <DropDown
            key={key}
            label={key}
            data={this.state.filteredHostels} 
            updateHostel={hostel => this.updateHostel(hostel)}
          />
        );
      ...
      return (
        <DropDown
          ...
          cityChange={city => this.cityChange(city)}
        />
      );
    }
  };
  ...
  show = (data, id) => {
    if (id) {
      ...
      if (obj) {
        return <Text style={{ fontSize: 18 }}>{obj.name}</Text>;
      }
    }
    return (
